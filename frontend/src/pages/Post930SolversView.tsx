    ...
    <div className="space-y-8 py-2 animate-fade-in">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-navy-800">
              {loading ? (
                ...
                filteredStudents.map((st: any) => (
                  <tr key={st.student_id} className="hover:bg-slate-50/50 dark:hover:bg-navy-850 transition-colors">
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 animate-fade-in">
          <div className="w-full max-w-lg rounded-3xl bg-white dark:bg-navy-950 border border-slate-200 dark:border-navy-700 shadow-lg overflow-hidden text-slate-900 dark:text-slate-100 space-y-4 p-6">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-navy-800 pb-3">
              ...
              <button
                ...
                className="p-1.5 rounded-xl bg-slate-100 dark:bg-navy-800 text-slate-400 hover:text-white"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="grid grid-cols-2 gap-3 bg-slate-50 dark:bg-navy-800 p-3 rounded-2xl border text-xs">
              <div>
                <span className="text-[10px] text-slate-400 font-bold uppercase">Official Score:</span>
                <p className="font-black text-slate-900 dark:text-white">{selectedStudent.official_locked_solved}</p>
              </div>
              <div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                {selectedStudent.problems?.map((p: any, idx: number) => (
                  <div key={idx} className="p-3 rounded-2xl bg-slate-50 dark:bg-navy-800 border flex items-center justify-between">
                    <div>
                      <p className="text-xs font-bold text-slate-900 dark:text-white">{idx + 1}. {p.problem_name || p.name}</p>
            ...
            <div className="pt-2 flex justify-end">
              <button
                onClick={() => setSelectedStudent(null)}
                className="px-4 py-2 rounded-xl bg-slate-200 dark:bg-navy-800 text-xs font-bold"
              >
                Close
              </button>
            </div>
