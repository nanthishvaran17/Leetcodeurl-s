...
const PageSkeleton = () => (
  <div className="w-full max-w-[1600px] mx-auto space-y-6 p-4 sm:p-6 animate-pulse">
    {/* Skeleton Top Bar */}
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-200/60 dark:border-navy-800/60">
      <div className="space-y-2">
        <div className="h-7 w-48 bg-slate-200 dark:bg-navy-800 rounded-xl"></div>
        <div className="h-3.5 w-72 bg-slate-200/70 dark:bg-navy-800/70 rounded-lg"></div>
      </div>
      <div className="flex gap-2">
        <div className="h-9 w-24 bg-slate-200 dark:bg-navy-800 rounded-xl"></div>
        <div className="h-9 w-32 bg-slate-200 dark:bg-navy-800 rounded-xl"></div>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="p-5 rounded-2xl bg-white dark:bg-navy-900 border border-slate-200/80 dark:border-navy-800/80 shadow-sm space-y-3">
          <div className="flex justify-between items-center">
            <div className="h-3.5 w-24 bg-slate-200 dark:bg-navy-800 rounded"></div>
            <div className="h-7 w-7 bg-slate-200 dark:bg-navy-800 rounded-xl"></div>
          </div>
          <div className="h-8 w-20 bg-slate-300 dark:bg-navy-700 rounded-lg"></div>
          <div className="h-3 w-32 bg-slate-200/60 dark:bg-navy-800/60 rounded"></div>
        </div>
      ))}
    </div>

    {/* Main Table/Chart Area Skeleton */}
    <div className="p-6 rounded-3xl bg-white dark:bg-navy-900 border border-slate-200/80 dark:border-navy-800/80 shadow-sm space-y-4">
      <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-navy-800">
        <div className="h-5 w-40 bg-slate-200 dark:bg-navy-800 rounded-lg"></div>
        <div className="h-8 w-48 bg-slate-200 dark:bg-navy-800 rounded-xl"></div>
      </div>
      <div className="space-y-3 pt-2">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="h-12 w-full bg-slate-100 dark:bg-navy-950/60 rounded-xl flex items-center justify-between px-4">
            <div className="h-4 w-36 bg-slate-200 dark:bg-navy-800 rounded"></div>
            <div className="h-4 w-24 bg-slate-200 dark:bg-navy-800 rounded"></div>
            <div className="h-4 w-16 bg-slate-200 dark:bg-navy-800 rounded"></div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-navy-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200 overflow-x-hidden w-full relative">
      ...
      {isNavigating && (
        <div className="fixed top-0 left-0 right-0 z-[100050] h-[3px] bg-brand-500/10 dark:bg-brand-400/10 pointer-events-none">
          <div className="h-full bg-gradient-to-r from-brand-500 via-indigo-500 to-sky-400 shadow-[0_0_12px_rgba(59,130,246,0.9)] animate-top-loader" />
        </div>
      )}
