...
export const ExportStatus: React.FC<ExportStatusProps> = ({ state, onRetry, onClose }) => {
  ...
  return (
    <div className="fixed bottom-[max(1rem,env(safe-area-inset-bottom,1rem))] right-4 left-4 sm:left-auto z-50 p-4 bg-slate-900 rounded-xl shadow-2xl text-white max-w-[calc(100vw-2rem)] sm:max-w-sm w-full border border-slate-700 font-sans">
      <div className="flex justify-between items-start mb-2">
        ...
        {onClose && (
          <button onClick={onClose} className="text-slate-400 hover:text-white transition-colors p-1 -mr-1 rounded-md touch-target-min flex items-center justify-center shrink-0" aria-label="Close">
      ...
      {isGenerating && (
        <div className="mt-4">
          <div className="flex space-x-2 items-center text-sm text-slate-300">
            ...
            <span className="leading-snug">Please wait while we prepare your file...</span>
          </div>
      ...
      {isFailed && (
        <div className="mt-3">
          ...
          {state.downloadId && (
            <p className="text-xs font-mono text-slate-500 mt-2 text-wrap-safe break-all">
