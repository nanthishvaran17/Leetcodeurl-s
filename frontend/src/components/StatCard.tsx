...
export const StatCard: React.FC<StatCardProps> = ({
  ...
  const colorStyles = {
    blue: 'from-brand-500/10 to-indigo-500/10 text-brand-600 dark:text-brand-400 border-brand-200/60 dark:border-brand-800/40 group-hover:bg-brand-500/20',
  ...
  if (loading) {
    return (
      <div className="glass-card p-5 rounded-3xl border border-slate-200 dark:border-navy-800 animate-pulse">
  ...
  return (
    <motion.div
      ...
      className={`glass-card p-5 sm:p-6 rounded-3xl border border-slate-200/80 dark:border-navy-800/80 card-ai-control group relative overflow-hidden shadow-lg ${onClick ? 'cursor-pointer' : ''}`}
    >
      <div className="flex items-center justify-between">
        <div>
