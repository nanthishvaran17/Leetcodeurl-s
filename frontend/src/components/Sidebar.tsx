...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <aside className="sticky top-20 w-60 xl:w-64 shrink-0 glass-card h-[calc(100vh-6rem)] border border-gray-200/80 dark:border-navy-800/80 rounded-3xl p-4 flex flex-col justify-between shadow-xl backdrop-blur-xl bg-white/80 dark:bg-navy-900/80 hidden lg:flex transition-all duration-300">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative w-full flex items-center justify-between px-3.5 py-2.5 rounded-2xl font-bold text-xs text-left transition-all duration-250 group cursor-pointer ${
                  isActive
              ...
              >
                ...
                {isActive && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-6 bg-amber-400 rounded-r-full shadow-[0_0_8px_#fbbf24] animate-pulse"></span>
                )}
