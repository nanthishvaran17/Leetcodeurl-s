...
const QBadge = ({ solved, q }: { solved: boolean; q: number }) => {
  ...
  return solved
    ...
    : <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-800 text-gray-500 border border-gray-700 text-[11px] font-bold"><XCircle className="w-3 h-3" />Q{q}</span>;
};
...
const statusBadge = (status: string) => {
  const map: Record<string, string> = {
    ...
    NOT_ATTENDED: 'bg-gray-800 text-gray-400 border-gray-700',
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10 bg-gradient-to-r from-slate-950 to-navy-950 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] font-mono text-gray-400">{student.reg_no}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-xl bg-white/10 hover:bg-white/20 flex items-center justify-center text-gray-400 hover:text-white transition-all cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 p-6 space-y-5">
          ...
          <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <span className="text-[10px] font-bold uppercase text-gray-500 tracking-wider">Student Profile</span>
              {student.username && (
                <a
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px]">
              ...
              {student.profile_rank && student.profile_rank !== '—' && (
                <div>
                  <span className="text-gray-500 block">LC Global Rank</span>
                  <span className="text-amber-300 font-bold">#{student.profile_rank}</span>
                </div>
              )}
              {student.profile_total_solved && (
                <div>
                  <span className="text-gray-500 block">Total LCProblems</span>
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-[11px]">
                  <div className="p-2 rounded-xl bg-black/30">
                    ...
                    <span className="text-2xl font-mono font-black text-white">{totalSolved}<span className="text-gray-500 text-sm">/4</span></span>
                  </div>
                  <div className="p-2 rounded-xl bg-black/30">
                    <span className="text-gray-500 block">LC Rank</span>
          ...
          {comparisonData && (
            <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-[11px]">
                <div>
                  <span className="text-gray-500 block">{comparisonData.previousWeek?.contestName || 'Prev Contest'}</span>
                  <span className="font-bold text-gray-300">{comparisonData.previousWeek?.publicAttended || 0} participated</span>
                </div>
                <div>
                  <span className="text-gray-500 block">{comparisonData.currentWeek?.contestName || 'This Contest'}</span>
                  <span className="font-bold text-gray-300">{comparisonData.currentWeek?.publicAttended || 0} participated</span>
                </div>
              </div>
          ...
          {/* Source info */}
          <div className="p-3 rounded-xl bg-white/3 border border-white/5 text-[10px] text-gray-500 space-y-1 font-mono">
