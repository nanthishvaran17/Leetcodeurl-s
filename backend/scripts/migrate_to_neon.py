...
def main():
    print("=" * 70, flush=True)
    print("NANDHA LEETCODE - AUTHORITATIVE NEON POSTGRES MIGRATION PIPELINE", flush=True)
    print("=" * 70, flush=True)
    ...
    for table in Base.metadata.sorted_tables:
        try:
            ...
            created_count += 1
            print(f"      [SCHEMA] Created table: {table.name}", flush=True)
    ...
    print(f"\n[4/5] Migrating {len(ordered_tables)} tables in dependency order...", flush=True)
    total_migrated = 0
    ...
    sqlite_conn.close()
    print("=" * 70, flush=True)
    print(f"COMPLETE! Total migrated records: {total_migrated:,}", flush=True)
    print("=" * 70, flush=True)
