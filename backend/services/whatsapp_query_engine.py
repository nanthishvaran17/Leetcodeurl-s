...
class WhatsAppQueryEngine:
    ...
    def get_overview(cls, db: Session, identity: WhatsAppIdentity) -> Dict[str, Any]:
        ...
        if identity.role == "UNREGISTERED":
            return {
                ...
                "message": (
                    ...
                    "2. Go to Profile Settings Link WhatsApp.\n"
        ...
        if identity.role == "STUDENT":
            ...
            msg = (
                ...
                f" Reg No: `{student.reg_no}` | Dept: {identity.department_code}\n\n"
        ...
        # B. FACULTY OVERVIEW
            ...
            if total_mentees == 0:
                msg = (
                    ...
                    f"ℹ You have no mentees currently assigned.\n"
            ...
            msg = (
                ...
                f" *Need Mentoring Attention:* {needing_attention} students\n\n"
        ...
        # C. HOD OVERVIEW
            ...
            msg = (
                f" *Department Command Summary — HOD {dept_code}*\n"
                f"‍ HOD: {identity.name}\n\n"
        ...
        # D. PRINCIPAL / SUPER ADMIN OVERVIEW
            ...
            msg = (
                f" *Nandha Institutional Command Overview*\n"
    ...
    def get_weekly_contest(cls, db: Session, identity: WhatsAppIdentity) -> Dict[str, Any]:
        ...
        if not latest_session:
            return {"success": True, "message": "ℹ No Weekly Contest session recorded yet."}
    ...
    def get_mentees_or_workload(cls, db: Session, identity: WhatsAppIdentity) -> Dict[str, Any]:
        ...
        if identity.role == "FACULTY":
            mentee_ids = identity.assigned_student_ids
            if not mentee_ids:
                return {"success": True, "message": "ℹ You have no assigned mentees."}
            ...
            for fac in faculty_list:
                ...
                status = " Normal" if assigned_count < 20 else (" At Ratio" if assigned_count == 20 else " Above Ratio")
    ...
    def search_student(
        ...
        if not matches:
            if identity.role == "STUDENT":
                ...
                return {"success": False, "message": f"ℹ No student matching '{term}' found in your assigned mentees."}
            elif identity.role == "HOD":
                return {"success": False, "message": f"ℹ No student matching '{term}' found in the Department of {identity.department_code}."}
            return {"success": False, "message": f"ℹ No student matching '{term}' found."}
