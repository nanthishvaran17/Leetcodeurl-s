...
def calculate_student_risk_engine(db: Session, student: Student, override_weights: Dict[str, float] = None) -> Dict[str, Any]:
    ...
    default_w1, default_w2, default_w3 = 0.40, 0.35, 0.25
    
    if override_weights:
        w1 = override_weights.get("w1", default_w1)
        w2 = override_weights.get("w2", default_w2)
        w3 = override_weights.get("w3", default_w3)
    else:
        w1, w2, w3 = default_w1, default_w2, default_w3
        
    used_weights = {"w1": w1, "w2": w2, "w3": w3}

    evidence = []

    # Factor 1: Output Risk (w1)
    output_risk = 0.0
    if total_solved < 15:
        output_risk = 100.0
...
def update_or_create_risk_profile(db: Session, student: Student, override_weights: Dict[str, float] = None) -> StudentRiskProfile:
    ...
    """
    res = calculate_student_risk_engine(db, student, override_weights)
