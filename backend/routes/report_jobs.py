...
def get_report_job_status(
    ...
    if getattr(current_user, "role", "") != "admin":
        ...
        if getattr(current_user, "role", "") == "hod" and job.department_id and job.department_id != getattr(current_user, "department_id", ""):
            raise HTTPException(status_code=403, detail="Cross-department report access forbidden")
        if getattr(current_user, "role", "") == "faculty" and job.requested_by != getattr(current_user, "email", ""):
