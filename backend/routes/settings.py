...
def get_student_forensic_trace(
    ...
    Consumes SINGLE SOURCE OF TRUTH normalized report object to guarantee zero mismatch with PDF.
    """
    ...
    try:
        report_data = build_normalized_forensic_report(
            db=db,
            search=clean_search,
        ...
        err_msg = str(ve)
        status_c = 404 if "not found" in err_msg.lower() else 400
        raise HTTPException(status_code=status_c, detail=err_msg)
    except Exception as err:
        logger.error(f"Forensic trace generation error: {err}")
        raise HTTPException(status_code=500, detail=f"Current contest verification could not be completed: {err}")
